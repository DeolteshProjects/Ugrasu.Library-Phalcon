<?php
namespace Elios\Libraryreport\Controllers\Library\Irbis;
...
//Собираем строку поискового запроса
    function getQuery($Array) {
        if (isset($Array)) {
            (!empty($Array["author"])) ? $Author = $Array['author'] : $Author = null;
            (!empty($Array["title"])) ? $Title = $Array['title'] : $Title = null;
            (!empty($Array["keyWords"])) ? $WordKey = $Array['keyWords'] : $WordKey = null;
        }
        //Совляем запрос
        $Query = "";
        if (!empty($Author)) {
